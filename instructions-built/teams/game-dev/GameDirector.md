...
Specialty file: ./instructions-src/templates/game-director.md
-->
...
- デフォルトチーム（TEAM_NUM が空）: GameDirector, worker1-3
- チーム番号付き（TEAM_NUM=1 等）: GameDirector1, worker1-1 等
...
# リーダーの名前を動的に設定
if [ -z "$TEAM_NUM" ]; then
    LEADER_NAME="GameProducer"
else
    LEADER_NAME="GameProducer${TEAM_NUM}"
fi
