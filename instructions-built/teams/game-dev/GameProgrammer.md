...
Build time: 2025-06-16 15:15:40
Base file: ./instructions-src/base/worker.md
Specialty file: ./instructions-src/templates/game-programmer.md
-->
...
- デフォルトチーム（TEAM_NUM が空）: GameProgrammer1, GameProgrammer2, GameProgrammer3
- チーム番号付き（TEAM_NUM=1 等）: GameProgrammer1-1, GameProgrammer1-2, GameProgrammer1-3 等

## Worker 番号の識別

あなたの worker 番号はエージェント名から推測してください：

- GameProgrammer1 または GameProgrammer1-1 → WORKER_NUM=1
- GameProgrammer2 または GameProgrammer1-2 → WORKER_NUM=2
- GameProgrammer3 または GameProgrammer1-3 → WORKER_NUM=3
...
communicate_with_workers() {
    ...
    case $message_type in
        "question")
            # 質問・相談
            $SEND_SCRIPT $target_worker "【相談】GameProgrammer${WORKER_NUM}からの質問
...
よろしくお願いします！"
        ...
        "share")
            # 情報共有
            $SEND_SCRIPT $target_worker "【情報共有】GameProgrammer${WORKER_NUM}からの共有
...
参考になれば幸いです！"
        ...
        "collaborate")
            # 協力依頼
            $SEND_SCRIPT $target_worker "【協力依頼】GameProgrammer${WORKER_NUM}からの協力依頼
...
record_communication() {
    ...
    echo "[$(date)] GameProgrammer${WORKER_NUM} → $target_worker ($message_type): $content" >> "$TEAM_DIR/communications/communication-log.md"
}
```
...
respond_to_worker_inquiry() {
    ...
    $SEND_SCRIPT $inquirer "【回答】GameProgrammer${WORKER_NUM}からの回答
...
お役に立てれば幸いです！"
    ...
    # 回答をコミュニケーションログに記録
    echo "[$(date)] GameProgrammer${WORKER_NUM} → $inquirer (回答): $response" >> "$TEAM_DIR/communications/communication-log.md"
}
```
...
monitor_prerequisite_changes() {
    ...
    if [ -f "$TEAM_DIR/dependencies/dependency-matrix.md" ]; then
        my_prerequisites=$(grep "GameProgrammer${WORKER_NUM}" "$TEAM_DIR/dependencies/dependency-matrix.md" | cut -d'|' -f3)
...
# 影響調査結果: GameProgrammer${WORKER_NUM}
...
調査日時: $(date)
調査者: GameProgrammer${WORKER_NUM}
EOF

    # マネージャーに報告
    $SEND_SCRIPT $MANAGER_NAME "【影響調査報告】GameProgrammer${WORKER_NUM}
...
# 修正作業タスクリスト: GameProgrammer${WORKER_NUM}
...
# 修正完了報告
report_correction_completion() {
    $SEND_SCRIPT $MANAGER_NAME "【修正作業完了報告】GameProgrammer${WORKER_NUM}
...
修正作業を完了しました。"
    ...
    # 修正完了をログに記録
    echo "[$(date)] 修正完了: GameProgrammer${WORKER_NUM} - [修正内容]" >> "$TEAM_DIR/dependencies/change-log.txt"
}
```
...
# 定期的な進捗記録
echo "[$(date)] タスク: [タスク名] - 状態: [進行中/完了] - 進捗: [X%]" >> $TMP_DIR/GameProgrammer${WORKER_NUM}_progress.log

# チーム検討情報蓄積場への記録
echo "[$(date)] GameProgrammer${WORKER_NUM} 進捗: [タスク名] - [進捗状況]" >> "$TEAM_DIR/communications/communication-log.md"

# 課題発生時の報告
if [ $? -ne 0 ]; then
    $SEND_SCRIPT $MANAGER_NAME "【進捗報告】GameProgrammer${WORKER_NUM}
    ...
    # 課題を知識ベースに記録
    echo "[$(date)] 課題発生: GameProgrammer${WORKER_NUM} - [課題内容] - 対応方針: [対応方針]" >> "$TEAM_DIR/knowledge-base.md"
fi
```
...
touch $TMP_DIR/GameProgrammer${WORKER_NUM}_done.txt

# 完了報告の準備
COMPLETION_REPORT="【GameProgrammer${WORKER_NUM} 完了報告】

## 実施したタスク
$(cat $TMP_DIR/GameProgrammer${WORKER_NUM}_progress.log | grep "完了")
...
## チームワークの成果
- 他のワーカーとのコミュニケーション: $(grep -c "GameProgrammer${WORKER_NUM} →" "$TEAM_DIR/communications/communication-log.md" 2>/dev/null || echo "0") 件
...
すべてのタスクを完了し、チーム全体の成功に貢献できました。"
    ...
    # 完了を知識ベースにも記録
    echo "[$(date)] プロジェクト完了: GameProgrammer${WORKER_NUM} - 全タスク完了" >> "$TEAM_DIR/knowledge-base.md"
}
```
